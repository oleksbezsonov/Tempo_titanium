...
function getListMessage(){
		...
		var httpparams = {
				...
				userid:userInfo.user_id,
			 	eventpostid: record.eventPostId,
			 	offset: offset
			}
		};
		...
		isInProcess = true;
	 	...
	 	httpClient.callPostAPI("getComments",httpparams,getListMessageCallback);
		 
	}catch(ex){
		Alloy.Globals.displayError("getListMessage " + JSON.stringify(ex));
	}finally{
	...
	try{
		...
		if(response.success == 1){
			...
			for(var i=0;i<size;i++){
				var item = records[i];
				var chatView;
				if(item.commented_byuserid == userInfo.user_id){
...
$.commentView.addEventListener("close", function(e){
	clearInterval(timer);
...
function addComments(text){
	try{
		...
		var httpparams = {
	 	...
	 	var httpClient = require("HttpConnection");
	 	httpClient.callPostAPI("addCommentToEvent",httpparams,function(data){
	 		try{
				Alloy.Globals.loading.hide();
				var response = JSON.parse(data);
