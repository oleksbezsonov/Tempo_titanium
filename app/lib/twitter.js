...
exports.Twitter = (function(global) {
  ...
  var Twitter = function(options) {
    ...
    self.listeners = {};
    self.parentwin = options.parentWindow;
  ...
  function createAuthWindow() {
	...
	try{
		self.parentwin.containingTab.open(webViewWindow);
	}catch(ex){
		try{
			...
			// navigationWrappingWindow = Ti.UI.createWindow({
				// title : ''
			// });
			navigation = Titanium.UI.iOS.createNavigationWindow({
			...
			Ti.API.error(" open twitter ex1  " + ex1);
		}
		//Ti.API.error(" open twitter,  " + ex);
	}
    closeButton.addEventListener('click', function(e) {
	      try{
	      	webViewWindow.close();
	      	navigation.close();
	      	//navigationWrappingWindow.close();
	      	
	      }catch(ex){
	      	Ti.API.error("closeButton " +ex);
	      }
